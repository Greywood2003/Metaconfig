...
describe('Location e2e test', () => {
  ...
  describe('new Location page', () => {
    ...
    it('should create an instance of Location', () => {
      cy.get(`[data-cy="streetAddress"]`).type('shaft quip quaintly');
      cy.get(`[data-cy="streetAddress"]`).should('have.value', 'shaft quip quaintly');

      cy.get(`[data-cy="postalCode"]`).type('acidly afford via');
      cy.get(`[data-cy="postalCode"]`).should('have.value', 'acidly afford via');

      cy.get(`[data-cy="city"]`).type('Lake Jailyn');
      cy.get(`[data-cy="city"]`).should('have.value', 'Lake Jailyn');

      cy.get(`[data-cy="stateProvince"]`).type('tall because among');
      cy.get(`[data-cy="stateProvince"]`).should('have.value', 'tall because among');
